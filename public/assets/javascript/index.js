...
async function delay(ms) {
  try {
    ...
    console.error("an error shouldn't be possible here");
    console.error(error);
  }
}
...
async function handleCreateRace() {
  ...
  if (!player_id || !track_id) {
    alert('Please select both a player and track to start the race!');
    return;
  }
  ...
  try {
    ...
    console.error('There was a problem in the handleCreateRace function ::', err);
  }
}

function runRace(raceID) {
  return new Promise(resolve => {
    // NOTE - use Javascript's built in setInterval method to get race info every 500ms
    let raceInterval = setInterval(async () => {
      try {
        ...
        if (race.status === 'in-progress') {
          renderAt('#leaderBoard', raceProgress(race.positions)); // to update the racer leaderboard
        } else {
        ...
        console.error('Problem with runRace function ::', err);
      }
    }, 500);
  });
}
...
async function runCountdown() {
  try {
    ...
    console.error('Problem with runCountdown function ::', err);
  }
}
